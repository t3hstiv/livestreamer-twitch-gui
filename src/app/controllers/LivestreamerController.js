...
export default Controller.extend( ChannelSettingsMixin, {
	metadata: service(),
	store   : service(),
	settings: service(),
	chat    : service(),
	...
	validateLivestreamer: function( exec ) {
		...
		return new Promise(function( resolve, reject ) {
			...
			later( onTimeout, time );
		})
			.then(function( version ) {
				kill();
				return version === getMax([ version, minimum ])
	...
	refreshStream: function( livestreamer ) {
		...
		promise.then(function() {
			later( this, this.refreshStream, livestreamer, interval );
		}.bind( this ) );
	},
